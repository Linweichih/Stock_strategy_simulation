...
class StockWebInfoParser:
    def __init__(self, stock_no):
        self.stock_no = stock_no

    # http://www.twse.com.tw/exchangeReport/STOCK_DAY?date=20180817&stockNo=2330  取2330一個月的股價與成交量
    def get_stock_history(self, date):
        quotes = []
        url = 'http://www.twse.com.tw/exchangeReport/STOCK_DAY?date=%s&stockNo=%s' % (date, self.stock_no)
        r = requests.get(url)
        data = r.json()
        return transform(data['data'])  # 進行資料格式轉換

    def create_df(self, date):
        s = pd.DataFrame(self.get_stock_history(date))
        s.columns = ['date', 'shares', 'amount', 'open', 'high', 'low', 'close', 'change', 'turnover']
        # "日期","成交股數","成交金額","開盤價","最高價","最低價","收盤價","漲跌價差","成交筆數"
        stock = []
        for i in range(len(s)):
            stock.append(self.stock_no)
...
if __name__ == '__main__':
    ...
    list_stock = ['2330']
    for stock_num in list_stock:
        StockWebInfoParser = StockWebInfoParser(int(stock_num))
        result = StockWebInfoParser.create_df('20210401')
        print(result)
